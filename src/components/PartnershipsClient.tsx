...
export function PartnershipsClient({
  ...
  const totalContacts = statsSource.length;
  const activePipeline = statsSource.filter((s) => s.stage !== "Closed" && s.stage !== "New").length;
  const inNegotiation = statsSource.filter((s) => s.stage === "Negotiating").length;
  const closedDeals = statsSource.filter((s) => s.stage === "Closed").length;
